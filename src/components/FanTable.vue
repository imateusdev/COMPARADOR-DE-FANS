  ...
  <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl overflow-hidden">
    ...
    <div class="p-4 border-b dark:border-gray-700">
      <div class="relative">
        <input
          ...
          class="w-full px-4 py-2 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent transition-all"
          @input="handleSearch"
        />
    ...
    <div class="p-4 border-b dark:border-gray-700 space-y-4">
      <div class="flex flex-wrap items-center gap-4">
        ...
        <button
          @click="clearFilters"
          class="px-4 py-2 text-sm font-medium rounded-lg transition-all transform hover:scale-105 bg-gradient-to-r from-red-500 to-pink-500 text-white shadow-lg hover:from-red-600 hover:to-pink-600"
    ...
    <!-- Paginação -->
    <div
      class="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 border-t dark:border-gray-700"
    >
      <div class="flex flex-wrap items-center justify-between gap-4">
        <span class="text-sm text-gray-700 dark:text-gray-300">
          Mostrando {{ Math.min(startIndex + 1, filteredFans.length) }} -
          {{ Math.min(endIndex, filteredFans.length) }} de
          {{ filteredFans.length }} itens
        </span>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const filteredFans = computed(() => {
      ...
      return [...props.fans].sort((a, b) => {
        const columnIndex = columns.findIndex(
          col => col.key === sortConfig.value.key
        );
    ...
    const totalPages = computed(() =>
      Math.max(1, Math.ceil(filteredFans.value.length / itemsPerPage.value))
    );
    const startIndex = computed(
      () => (currentPage.value - 1) * itemsPerPage.value
    );
    const endIndex = computed(() =>
      Math.min(startIndex.value + itemsPerPage.value, filteredFans.value.length)
    );
    ...
    const handleItemsPerPageChange = () => {
      const newTotalPages = Math.ceil(
        filteredFans.value.length / itemsPerPage.value
      );
